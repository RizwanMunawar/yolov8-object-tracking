...
from yolo.utils import LOGGER, emojis, is_colab
...
def reset_model(key=''):
    ...
    api_key, model_id = split_key(key)
    r = requests.post('https://api.com/model-reset', json={"apiKey": api_key, "modelId": model_id})
...
def export_model(key='', format='torchscript'):
    ...
    r = requests.post('https://api.com/export',
                      json={
                          "apiKey": api_key,
                          "modelId": model_id,
                          "format": format})
...
def get_export(key='', format='torchscript'):
    ...
    r = requests.post('https://api.com/get-export',
                      json={
                          "apiKey": api_key,
                          "modelId": model_id,
                          "format": format})
